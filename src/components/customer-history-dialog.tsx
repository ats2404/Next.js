...
export function CustomerHistoryDialog({
  ...
  const getBalanceAfterTransaction = (index: number) => {
    // This is complex. For simplicity, let's just show the final balance.
    // A more accurate running balance requires processing all transactions up to that point.
    let runningBalance = balance;
    // Iterate backwards from the transaction *before* the current one to the start
                                ...
                                <CardContent className="p-3 grid grid-cols-3 items-center">
                                    <div className="col-span-1">
                                        <p className="text-xs text-muted-foreground">
                                            {format(new Date(tx.timestamp), 'dd MMM yy')}
                                        </p>
